...
describe("TaskEscrow:", function () {
    ...
    async function deployTaskEscrow() {
        ...
        await token.connect(taskOwner).approve(await taskEscrow.getAddress(), hre.ethers.parseUnits("500", 18));
    ...
    describe("emergencyWithdrawToken", function () {
        ...
        describe("Non-task token withdrawal", function () {
            it("Should allow factory to withdraw non-task tokens", async function () {
                ...
                expect(await secondToken.balanceOf(await taskEscrow.getAddress())).to.equal(tokenAmount);

                // Get factory's initial balance
                const initialFactoryBalance = await secondToken.balanceOf(factory.address);

                // Call emergencyWithdrawToken
                await taskEscrow.connect(factory).emergencyWithdrawToken(await secondToken.getAddress());

                // Verify tokens were transferred to factory
                const finalFactoryBalance = await secondToken.balanceOf(factory.address);
                expect(finalFactoryBalance).to.equal(initialFactoryBalance + tokenAmount);

                // Verify contract balance is now 0
                expect(await secondToken.balanceOf(await taskEscrow.getAddress())).to.equal(0);
            });
            ...
            it("Should emit EmergencyTokenWithdrawn event", async function () {
                ...
                await expect(taskEscrow.connect(factory).emergencyWithdrawToken(await secondToken.getAddress()))
            ...
            it("Should work with multiple different tokens", async function () {
                ...
                await expect(taskEscrow.connect(factory).emergencyWithdrawToken(await thirdToken.getAddress()))
                    .to.emit(taskEscrow, "EmergencyTokenWithdrawn")
                    .withArgs(await thirdToken.getAddress(), amount2);

                // Verify both balances are 0
                expect(await secondToken.balanceOf(await taskEscrow.getAddress())).to.equal(0);
                expect(await thirdToken.balanceOf(await taskEscrow.getAddress())).to.equal(0);
            });
        });
        ...
        describe("Task reward token withdrawal", function () {
            it("Should allow withdrawal of task reward token when task is PAID_OUT", async function () {
                ...
                await token.connect(taskOwner).transfer(await taskEscrow.getAddress(), additionalTokens);

                // Should allow withdrawal when status is PAID_OUT
                await expect(taskEscrow.connect(factory).emergencyWithdrawToken(await token.getAddress()))
                    .to.emit(taskEscrow, "EmergencyTokenWithdrawn")
            ...
            it("Should revert withdrawal of task reward token when task is OPEN", async function () {
                ...
                await expect(
                    taskEscrow.connect(factory).emergencyWithdrawToken(await token.getAddress())
                ).to.be.revertedWithCustomError(taskEscrow, "CANNOT_WITHDRAW_TASK_REWARD_TOKEN_WHILE_TASK_IS_ACTIVE");
            });
        ...
        describe("Access control and validation", function () {
            it("Should revert if called by non-factory", async function () {
                ...
                const secondToken = await deploySecondToken();
                await secondToken.mint(await taskEscrow.getAddress(), hre.ethers.parseUnits("100", 18));

                // Try to call from taskOwner
                await expect(
                    taskEscrow.connect(taskOwner).emergencyWithdrawToken(await secondToken.getAddress())
                ).to.be.revertedWithCustomError(taskEscrow, "CALLER_IS_NOT_FACTORY");

                // Try to call from taskAssignee
                await expect(
                    taskEscrow.connect(taskAssignee).emergencyWithdrawToken(await secondToken.getAddress())
                ).to.be.revertedWithCustomError(taskEscrow, "CALLER_IS_NOT_FACTORY");

                // Try to call from other account
                await expect(
                    taskEscrow.connect(otherAccount).emergencyWithdrawToken(await secondToken.getAddress())
                ).to.be.revertedWithCustomError(taskEscrow, "CALLER_IS_NOT_FACTORY");
            });
            ...
            it("Should revert if token address is zero", async function () {
                ...
                await expect(
                    taskEscrow.connect(factory).emergencyWithdrawToken(hre.ethers.ZeroAddress)
            ...
            it("Should revert if no tokens to withdraw", async function () {
                ...
                await expect(
                    taskEscrow.connect(factory).emergencyWithdrawToken(await secondToken.getAddress())
        ...
        describe("Edge cases and scenarios", function () {
            ...
            it("Should not affect other token balances", async function () {
                ...
                expect(await token3.balanceOf(await taskEscrow.getAddress())).to.equal(amount2);
            });
            ...
            it("Should work with tokens having different decimals", async function () {
                ...
                await expect(taskEscrow.connect(factory).emergencyWithdrawToken(await token6.getAddress()))
                    .to.emit(taskEscrow, "EmergencyTokenWithdrawn")
