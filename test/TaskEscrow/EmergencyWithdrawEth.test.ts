...
describe("TaskEscrow:", function () {
  ...
  describe("emergencyWithdrawETH", function () {
    it("Should allow factory to withdraw ETH when balance exists", async function () {
      ...
      const initialFactoryBalance = await hre.ethers.provider.getBalance(
        factory.address
      );
      ...
      const finalFactoryBalance = await hre.ethers.provider.getBalance(
        factory.address
      );
      expect(finalFactoryBalance).to.equal(
        initialFactoryBalance - gasUsed + ethAmount
      );
    ...
    it("Should not affect ERC20 token balances", async function () {
      ...
      expect(await token.balanceOf(await taskEscrow.getAddress())).to.equal(
        initialTokenBalance
      );
