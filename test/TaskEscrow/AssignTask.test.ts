...
describe("TaskEscrow:", function () {
  ...
  describe("assignTask", function () {
    it("Should assign task successfully by task owner", async function () {
      ...
      await expect(tx).to.emit(taskEscrow, "TaskStatusChanged").withArgs(0, 1); // OPEN -> ASSIGNED
    ...
    it("Should revert if called by non-task owner", async function () {
      const { taskEscrow, taskAssignee, otherAccount } =
        await getInitializedContract();
    ...
    it("Should revert if task is already assigned", async function () {
      ...
      await expect(
        ...
        taskEscrow,
        "TASK_HAS_ALREADY_BEEN_ASSIGNED_OR_COMPLETED"
      );
    });
    ...
    it("Should not allow assignment to zero address", async function () {
      ...
      await expect(
        taskEscrow.connect(taskOwner).assignTask(hre.ethers.ZeroAddress)
      ).to.be.revertedWithCustomError(
        taskEscrow,
        "CAN_NOT_ASSIGN_TO_ADDRESS_ZERO"
      );
    });
    ...
    it("Should prevent task owner from assigning to themselves", async function () {
      ...
      await expect(
        taskEscrow.connect(taskOwner).assignTask(taskOwner.address)
